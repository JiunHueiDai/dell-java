...
public class TicTacToe {
	...
	public static void main(String[] args) {
		...
		while (continueGame) {
			...
			if (isValidInput(xyChar)) {
				...
				System.out.println(hand + "Please input a valid hand such as 00, 01, 10, or so on.");
			}
		}
	...
	private static boolean isValidInput(char[] xy) {
		if (xy.length != 2) {
			return false;
		}

		try {
			Integer.parseInt(Character.toString(xy[0]));
			Integer.parseInt(Character.toString(xy[1]));
